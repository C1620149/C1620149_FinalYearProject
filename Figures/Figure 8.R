...
jpeg("Figures/Figure8a.jpg", width = 500, height = 500, res=100)
ggline(DataGraphs, x = "Individualism", y = "Likelihood_of_Failure", linetype = "Task_Condition", 
       add = "mean_se", order = c("Low", "High"), ylab = "Mean Likelihood of Failure", xlab = "Individualism", 
       xlim = c(1,2), ylim = c(1,9), ) + scale_y_continuous(breaks=c(1, 2, 3, 4, 5, 6, 7, 8, 9), labels=c("-4", "-3", "-2", "-1", "0", "1", "2", "3", "4")) +
  theme(legend.position=c(.5, .87)) +
  labs(linetype="Task Condition")
dev.off() 

# Individualism x Task Condition x Affirmation Condition Plot
jpeg("Figures/Figure8b.jpg", width = 500, height = 500, res=100)
DataGraphs$Combo <- paste0(DataGraphs$Affirmation_Condition, DataGraphs$Task_Condition)
ggline(DataGraphs, x = "Individualism", y = "Likelihood_of_Failure", linetype = "Combo", color = "Combo",
       add = "mean_se", order = c("Low", "High"), ylab = "Mean Likelihood of  Failure", xlab = "Individualism", 
       xlim = c(1,2), ylim = c(1,9), ) + scale_y_continuous(breaks=c(1, 2, 3, 4, 5, 6, 7, 8, 9), labels=c("-4", "-3", "-2", "-1", "0", "1", "2", "3", "4")) +
...
# Threat Plot
jpeg("Figures/Figure8c.jpg", width = 500, height = 500, res=100)
ggline(DataGraphs, x = "Threat_Condition", y = "Likelihood_of_Failure", 
       add = "mean_se", order = c("No Threat", "Threat"), ylab = "Mean Likelihood of Failure", xlab = "Threat Condition", 
       xlim = c(1,2), ylim = c(1,9), ) + scale_y_continuous(breaks=c(1, 2, 3, 4, 5, 6, 7, 8, 9), labels=c("-4", "-3", "-2", "-1", "0", "1", "2", "3", "4"))
dev.off() 

# Threat x Affirmation Plot
jpeg("Figures/Figure8d.jpg", width = 500, height = 500, res=100)
ggline(DataGraphs, x = "Threat_Condition", y = "Likelihood_of_Failure", color = "Affirmation_Condition", 
       add = "mean_se", order = c("No Threat", "Threat"), ylab = "Mean Likelihood of Failure", xlab = "Threat Condition", 
       xlim = c(1,2), ylim = c(1,9), ) + scale_y_continuous(breaks=c(1, 2, 3, 4, 5, 6, 7, 8, 9), labels=c("-4", "-3", "-2", "-1", "0", "1", "2", "3", "4")) +
  theme(legend.position=c(.5, .87)) +
